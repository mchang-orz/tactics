...
func GetOffsets(anchor:Control.LayoutPreset, offset: Vector2) -> Rect2:
	...
	match anchor:
		...
		Control.PRESET_CENTER_TOP, Control.PRESET_CENTER_BOTTOM, Control.PRESET_CENTER:
			retValue.position.x = -0.5 * self.size.x
	...
	match anchor:
		...
		Control.PRESET_CENTER_LEFT, Control.PRESET_CENTER_RIGHT, Control.PRESET_CENTER:
			retValue.position.y = -0.5 * self.size.y
...
func SnapAnchor(myAnchor:Control.LayoutPreset,parentAnchor:Control.LayoutPreset, offset:Vector2):
	var parentVector:Vector2 = GetParentAnchor(parentAnchor)
	var myOffsets:Rect2 = GetOffsets(myAnchor, offset)
...
func MoveAnchorToPosition(myAnchor:Control.LayoutPreset, parentAnchor:Control.LayoutPreset, 
offset:Vector2, duration:float, 
trans:Tween.TransitionType=Tween.TRANS_LINEAR, anchorEase:Tween.EaseType=Tween.EASE_IN_OUT):
	var parentVector:Vector2 = GetParentAnchor(parentAnchor)
	var myOffsets = GetOffsets(myAnchor, offset)
...
func ToAnchorPosition(anchor:PanelAnchor,animated:bool):
	if animated:
		await MoveAnchorToPosition(anchor.myAnchor,anchor.parentAnchor,anchor.offset,anchor.duration,anchor.trans,anchor.anchorEase)
	else:
		SnapAnchor(anchor.myAnchor,anchor.parentAnchor,anchor.offset)
