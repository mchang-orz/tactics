...
signal RemovedNotification(StatusEffect)

func Add(status_effect:GDScript, status_condition:GDScript, 
effect_name:String = "Status Effect", condition_name:String = "Status Condition"):
...
func Remove(target:StatusCondition):
	...
	condition.assign(children.filter(func(node): 
		return node is StatusCondition && !node.is_queued_for_deletion()))
	if condition.is_empty():
		RemovedNotification.emit(effect)
		effect.queue_free()
